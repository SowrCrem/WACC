...
object macroPreProcessor {
  ...
  /**
    ...
    * @param macroDef
    */
  private def addToMacroMap(macroDef: String): Unit = {
  ...
  /**
    ...
    */
  private def makeMacroMap(sourceCode: String): Either[String, List[String]] = {
    ...
    while (i < numLines) {
      ...
      if (relevantContent.startsWith("~DEF")) {
        ...
        return Right(lines.slice(i, lines.size).toList)
  ...
  def preprocess(sourceCode: String): Either[String, String] = {
    ...
    makeMacroMap(sourceCode) match {
      ...
      case Right(x) => {
        ...
        Right(macroMap.foldLeft(sourceCode) { case (code, (name, replacement)) =>
